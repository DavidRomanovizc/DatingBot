...
async def questionnaires_keyboard(target_id: int, monitoring: bool = False) -> InlineKeyboardMarkup:
    ...
    dislike = InlineKeyboardButton(text='👎', callback_data=action_keyboard.new(action="dislike",
                                                                               target_id=target_id))
    report = InlineKeyboardButton(text='🔞', callback_data=action_keyboard.new(action="report",
                                                                              target_id=target_id))
    ...
    ban = InlineKeyboardButton(text=_("🚫 Забанить"),
                               callback_data=action_keyboard_monitoring.new(action="ban",
                                                                            target_id=target_id))
    next_btn = InlineKeyboardButton(text=_("Следующий"), callback_data=action_keyboard_monitoring.new(
        action="next",
        target_id=target_id))
    if not monitoring:
...
async def report_menu_keyboard(telegram_id: int) -> InlineKeyboardMarkup:
    markup = InlineKeyboardMarkup(row_width=1)
    adults_only = InlineKeyboardButton(text='🔞', callback_data=action_report_keyboard.new(
        action="adults_only",
        target_id=telegram_id
    ))
    drugs = InlineKeyboardButton(text='💊', callback_data=action_report_keyboard.new(
        action="drugs",
        target_id=telegram_id
    ))
    scam = InlineKeyboardButton(text='💰', callback_data=action_report_keyboard.new(
        action="scam",
        target_id=telegram_id
    ))
    another = InlineKeyboardButton(text='🦨', callback_data=action_report_keyboard.new(
        action="another",
        target_id=telegram_id
    ))
    cancel = InlineKeyboardButton(text='❌', callback_data=action_report_keyboard.new(
