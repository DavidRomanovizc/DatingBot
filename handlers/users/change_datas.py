...
async def start_change_data(call: CallbackQuery):
    await call.message.answer(f'Выберите, что вы хотите изменить: ', reply_markup=change_profile_kb)
...
async def change_sex(call: CallbackQuery, state: FSMContext):
    if call.data == 'male':
        try:
            ...
            await call.message.answer(f'Ваш новый пол: <b>Мужской</b>')
            await state.reset_state()
        except Exception as err:
            logger.error(err)
    ...
    if call.data == 'female':
        try:
            ...
            await call.message.answer(f'Ваш новый пол: <b>Женский</b>')
            await state.reset_state()
        except Exception as err:
            logger.error(err)
            await call.message.answer(f'Произошла неизвестная ошибка')
...
async def change_children(call: CallbackQuery, state: FSMContext):
    if call.data == 'true':
        try:
            ...
            await call.message.answer(f'Теперь у вас: <b>есть</b> дети')
            await state.reset_state()
        except Exception as err:
            logger.error(err)
            await call.message.answer(f'Произошла неизвестная ошибка')
    ...
    if call.data == 'false':
        try:
            ...
            await call.message.answer(f'Теперь у вас: <b>нет</b> детей')
            await state.reset_state()
        except Exception as err:
            logger.error(err)
            await call.message.answer(f'Произошла неизвестная ошибка')
...
async def change_home(call: CallbackQuery, state: FSMContext):
    if call.data == 'true':
        try:
            ...
            await call.message.answer(f'Теперь у вас: <b>есть</b> квартира')
            await state.reset_state()
        except Exception as err:
            logger.error(err)
    ...
    if call.data == 'false':
        try:
            ...
            await call.message.answer(f'Теперь у вас: <b>нет</b> квартиры')
            await state.reset_state()
        except Exception as err:
            logger.error(err)
            await call.message.answer(call.from_user.id, f'Произошла неизвестная ошибка')
            await state.reset_state()
...
async def change_education(call: CallbackQuery, state: FSMContext):
    if call.data == 'higher_edu':
        try:
            await db.update_user_apartment(apartment=True, telegram_id=call.from_user.id)
            await call.message.answer(f'Теперь у вас: <b>Высшее</b> образование')
            await state.reset_state()
        except Exception as err:
            logger.error(err)
            await call.message.answer(call.from_user.id, f'Произошла незвестная ошибка')
            await state.reset_state()
    if call.data == 'secondary_edu':
        try:
            await db.update_user_apartment(apartment=False, telegram_id=call.from_user.id)
            await call.message.answer(f'Теперь у вас: <b>Среднее</b> образование')
            await state.reset_state()
        except Exception as err:
            logger.error(err)
            await call.message.answer(f'Произошла незвестная ошибка')
            await state.reset_state()
...
async def change_style(call: CallbackQuery, state: FSMContext):
    if call.data == 'study_lifestyle':
        try:
            ...
            await call.message.answer(f'Теперь вы учитесь!')
            await state.reset_state()
        except Exception as err:
            logger.error(err)
            await call.message.answer(f'Произошла незвестная ошибка')
        ...
        try:
            ...
            await call.message.answer('Теперь вы работаете!')
            await state.reset_state()
        except Exception as err:
            logger.error(err)
            await call.message.answer(f'Произошла незвестная ошибка')
        ...
        try:
            ...
            await call.message.answer(f'Теперь вы ищете работу!')
            await state.reset_state()
        except Exception as err:
            logger.error(err)
            await call.message.answer(f'Произошла незвестная ошибка')
        ...
        try:
            ...
            await call.message.answer(f'Теперь вы домохозяин/домохозяйка!')
            await state.reset_state()
        except Exception as err:
            logger.error(err)
            await call.message.answer(f'Произошла незвестная ошибка')
