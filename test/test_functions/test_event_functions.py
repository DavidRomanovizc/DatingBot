...
async def test_check_event_date_past_date():
    ...
    with patch.object(db_commands, 'select_user_meetings',
        ...
        with patch.object(db_commands, 'update_user_meetings_data', new=update_mock) as mock:
            await extra_features.check_event_date(telegram_id)
            mock.assert_called_once_with(telegram_id=telegram_id, is_admin=False, verification_status=False,
                                         is_active=False)
...
async def test_check_event_date_future_date():
    ...
    with patch.object(db_commands, 'select_user_meetings',
        ...
        with patch.object(db_commands, 'update_user_meetings_data', new=update_mock) as mock:
            await extra_features.check_event_date(telegram_id)
            mock.assert_called_once_with(telegram_id=telegram_id, is_admin=True, verification_status=True,
                                         is_active=True)
